...
export function RobotDisplay2024({ entry }: ArmDisplayProps) {
  ...
  const draw = (context: CanvasRenderingContext2D, [_]: (number | undefined)[], frameCount: number) => {
    ...
    const wheelLeft = createShape(wheelDiameterMeters, wheelDiameterMeters, -0.27, 0.0, 'black');
    const wheelRight = createShape(wheelDiameterMeters, wheelDiameterMeters, 0.27, 0.0, 'black');
    ...
    const basePlate = createShape(basePlateWidth, basePlateheight, 0, basePlateHeightOffGround, 'silver');
    drawRoundRectangle(context, basePlate, basePlate.heightPixels / 3);
    ...
    const bumpers = createShape(bumbersWidth, bumbersHeight, bumpersOffset, bumperHeightOffGround, 'forestgreen');
