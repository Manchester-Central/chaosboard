...
export function NumberDisplayConfig({config, onChange}: NumberDisplayConfigProps) {
    ...
    return <div className='d-grid gap-2'>
            Digits: <input type='number' onChange={e => setDigits(+e.target.value)} value={digits}/>
        </div>;
}
    ...
    return (
        <div style={{ width: '100%', textAlign: 'center' }} className={'p-2'}>
            <Textfit mode="single" max={60} onClick={onClick}>
                {isEditable ? <small style={{fontSize: '0.3em', opacity: 0.6}}><FontAwesomeIcon icon={faPenToSquare} /></small> : <></>} <pre>{Intl.NumberFormat('en-US', {maximumSignificantDigits: configs?.digits ?? 5}).format(value)}</pre>
            </Textfit>
        </div>
    );
}